...
public record PaymentQueryResult(
  ...
  public static PaymentQueryResult from(Payment payment) {
    return PaymentQueryResult.builder()
        ...
        .paymentKey(
            payment.getPaymentDetail() == null ? null : payment.getPaymentDetail().getPaymentKey())
        .paymentMethod(
            payment.getPaymentDetail() == null
                ? null
                : payment.getPaymentDetail().getMethod().name())
        .paidAt(payment.getPaymentDetail() == null ? null : payment.getPaymentDetail().getPaidAt())
        .cancelReason(
            payment.getCancelDetail() == null ? null : payment.getCancelDetail().getReason())
        .canceledAt(
            payment.getCancelDetail() == null ? null : payment.getCancelDetail().getCanceledAt())
        .build();
  }
}
