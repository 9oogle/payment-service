  ...
  public ApproveResult request(PaymentId paymentId, String paymentKey, OrderDetail orderDetail) {
    ...
    if (res.getStatusCode().is2xxSuccessful()) {
      ...
      LocalDateTime paidAt =
          LocalDateTime.parse(
              node.get("approvedAt").asText(), DateTimeFormatter.ISO_OFFSET_DATE_TIME);
      ...
      log.info(
          "토스 결제 승인 성공 - HTTP 상태코드: {}, 주문 ID: {}, 결제 ID: {}, PaymentKey: {}, 결제금액: {}, 결제수단: {}, 승인상태: {}, 승인일시: {}",
          res.getStatusCode(),
          orderId,
          paymentId.getId(),
          paymentKey,
          approvedAmount,
          method,
          "%s(%s)".formatted(status, status.getDescription()),
          paidAt);
    ...
    log.error(
        "토스 결제 승인 실패 - HTTP 상태코드: {},주문 ID: {}, 결제 ID: {}, PaymentKey: {}, 결제금액: {}, 사유: {}",
        res.getStatusCode(),
        orderId,
        PaymentId.of().getId(),
        paymentKey,
    ...
    return ApproveResult.builder().success(false).reason(reason).build();
  }
}
