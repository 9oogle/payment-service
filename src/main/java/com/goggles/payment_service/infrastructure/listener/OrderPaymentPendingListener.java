...
public class OrderPaymentPendingListener {
    ...
    @KafkaListener(topics = "${topics.order.payment-pending}",
    ...
    public void onReceived(Message<String> message, Acknowledgment ack) throws Exception {
        ...
        try {
            OrderPaymentPending data = objectMapper.readValue(message.getPayload(),
                    OrderPaymentPending.class);

            paymentService.createPayment(PaymentServiceDto.Create.builder()
            ...
            log.info("결제 생성 성공 - 주문 ID, {}, 주문상품: {}, 결제금액: {}",
                    data.orderId(), data.orderName(), data.amount());
    ...
    @KafkaListener(topics = "${topics.order.payment-pending}-dlt",
            groupId = "payment-service")
